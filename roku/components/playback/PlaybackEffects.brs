...
sub StartActiveSlideMotionForScene(ctx as Object)
    StopMotionAnimationForScene(ctx)
    if ctx.photoMotion <> "kenBurns" then return
    if ctx.playlist.Count() = 0 then return

    entry = ctx.playlist[ctx.playlistIndex]
    if entry.type = "video" or entry.type = "livePhoto" then return

    node = ctx.mainPosters[ctx.ringCurrent]
    if node = invalid then return

    duration = ctx.slideDuration - ctx.activeTransitionDuration
    if duration <= 1 then return

    plan = ctx.activeMotionPlan
    if plan = invalid then
        plan = BuildPhotoMotionPlanForMode(ctx.photoMotion, entry, node, node.translation)
        ctx.activeMotionPlan = plan
    end if
    if plan = invalid then return

    node.scale = [plan.startScale, plan.startScale]
    node.translation = plan.startTranslation
    node.scaleRotateCenter = [Int(node.width / 2), Int(node.height / 2)]

    anim = CreateObject("roSGNode", "Animation")
    anim.duration = duration
    anim.repeat = false

    zoom = anim.createChild("Vector2DFieldInterpolator")
    zoom.fieldToInterp = "main" + ctx.ringCurrent.ToStr() + ".scale"
    zoom.key = [0.0, 1.0]
    zoom.keyValue = [[plan.startScale, plan.startScale], [plan.endScale, plan.endScale]]

    pan = anim.createChild("Vector2DFieldInterpolator")
    pan.fieldToInterp = "main" + ctx.ringCurrent.ToStr() + ".translation"
    pan.key = [0.0, 1.0]
    pan.keyValue = [plan.startTranslation, plan.endTranslation]

    ctx.top.appendChild(anim)
    anim.control = "start"
    ctx.activeMotionAnim = anim
end sub
...
function StartBgCrossfadeForScene(ctx as Object, duration as Float) as Boolean
    ...
    LogDebug(PlaybackLogScope(ctx), "StartBgCrossfade from=" + ctx.pendingBgFromSlot.ToStr() + " to=" + ctx.pendingBgToSlot.ToStr() + " duration=" + Str(duration).Trim())
